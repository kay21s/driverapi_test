...
void init_shared_memory()
{
	...
	if (ftruncate(fd, sizeof(CUcontext)+2) == -1)
		exit(-1);
		/* Handle error */;
	...
	s = shm;
	printf("setting the context with shared memory\n");
	context = shm;
	//memcpy(s+1, context, sizeof(CUcontext));
	*(s+sizeof(CUcontext)) = 'a';
}
